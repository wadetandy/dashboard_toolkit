...
class BuilderTest < ActionView::TestCase
  ...
  def test_initialize_should_require_context_and_elements
    assert_raise(ArgumentError) { DashboardToolkit::Breadcrumbs::Builder.new }
    assert_raise(ArgumentError) { DashboardToolkit::Breadcrumbs::Builder.new(@template) }
    assert_nothing_raised { DashboardToolkit::Breadcrumbs::Builder.new(@template, []) }
  end
  ...
  def test_initialize_should_set_context
    builder = DashboardToolkit::Breadcrumbs::Builder.new(@template, [])
    assert_equal(@template, builder.instance_variable_get(:'@context'))
  end

  def test_initialize_should_set_elements
    builder= DashboardToolkit::Breadcrumbs::Builder.new(@template, [1, 2])
  ...
  def test_compute_name_with_symbol
    builder = DashboardToolkit::Breadcrumbs::Builder.new(@template, [])
    element = DashboardToolkit::Breadcrumbs::Element.new(:method_for_name, nil)
  ...
  def test_compute_name_with_proc
    builder = DashboardToolkit::Breadcrumbs::Builder.new(@template, [])
    element = DashboardToolkit::Breadcrumbs::Element.new(proc { |template| template.send(:proc_for_name) }, nil)
  ...
  def test_compute_name_with_string
    builder = DashboardToolkit::Breadcrumbs::Builder.new(@template, [])
    element = DashboardToolkit::Breadcrumbs::Element.new("name from string", nil)
  ...
  def test_compute_path_with_symbol
    builder = DashboardToolkit::Breadcrumbs::Builder.new(@template, [])
    element = DashboardToolkit::Breadcrumbs::Element.new(nil, :method_for_path)
  ...
  def test_compute_path_with_proc
    builder = DashboardToolkit::Breadcrumbs::Builder.new(@template, [])
    element = DashboardToolkit::Breadcrumbs::Element.new(nil, proc { |template| template.send(:proc_for_path) })
  ...
  def test_compute_path_with_hash
    builder = DashboardToolkit::Breadcrumbs::Builder.new(@template, [])
    element = DashboardToolkit::Breadcrumbs::Element.new(nil, { :foo => "bar" })
  ...
  def test_compute_path_with_string
    builder = DashboardToolkit::Breadcrumbs::Builder.new(@template, [])
    element = DashboardToolkit::Breadcrumbs::Element.new(nil, "http://localhost/#string")
