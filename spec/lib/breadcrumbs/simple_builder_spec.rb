...
class SimpleBuilderTest < ActionView::TestCase
  ...
  def test_render_with_no_links
    elements = (1..2).collect do |index|
      DashboardToolkit::Breadcrumbs::Element.new("Element #{index}", nil)
    end
  ...
  def simplebuilder(*args)
    DashboardToolkit::Breadcrumbs::SimpleBuilder.new(*args)
  end

  def generate_elements(number)
    (1..number).collect do |index|
      DashboardToolkit::Breadcrumbs::Element.new("Element #{index}", "/element/#{index}")
    end
  end
